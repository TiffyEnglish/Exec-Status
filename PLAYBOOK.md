...
| Step  | Action                                   | Details                                                                                                                                                                                                                                                                                   |
| ----- | ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 | In Development ← Jira                | Sync `productReport.sections[]` where `id` = `in_dev` to filter 10566 (§3). For each `jiraIssues[]` row: refresh `eaf` from Jira before workload counts used for ETC / % Cmp. Prefer `npm run jira:workload` (REST) or MCP + §3 workload rules to set `todoCount`, `inProgressCount`, `codeReviewCount`, `qaReviewCount`, and `closedCount` (§3 EAF before ETC and % Cmp.).            |
...
3. Each epic/feature row has `jiraIssues[]` with keys; sync `eaf` per item 4 below before relying on workload counts (`todoCount`, `inProgressCount`, `codeReviewCount`, `qaReviewCount`, `closedCount` — §3 Workload) for ETC, % Cmp., or narrative that assumes those metrics are current.
...
Slide row order: `scripts/render.mjs` sorts `in_dev` rows by Project band first (Trackers → Integration, including Device Integration → Other → Ops), then by `projectGroup` string, then feature `name`. Ordering does not follow Jira filter rank—the checklist instead verifies keys / summaries match filter 10566.

Group epics by repeating `projectGroup` on consecutive rows (rowspan in the Project column)—e.g. Trackers, Device Integration, Ops, Other.
...
### Workload counts (epic children in Jira)

Per `jiraIssues[].key` (the epic), count issues returned by `"Epic Link" = KEY` (or `parent = KEY` if your site uses parent links). Classify each child:

| Stored field | Rule |
| --- | --- |
| `todoCount` | Status category To Do (`new`). |
| `codeReviewCount` | Status name contains `Code Review` (case-insensitive), e.g. Code Review 2, Code Review Passed. |
| `qaReviewCount` | Status name is exactly `QA Review` (case-insensitive). |
| `inProgressCount` | Status category In Progress (`indeterminate`) and not counted as Code Review or QA Review above. |
| `closedCount` | Status category Done (`done`) — e.g. Closed, Rejected, or any terminal done state. |

Buckets are mutually exclusive in this order: Closed → To Do → Code Review (name) → QA Review (name) → In Progress (remaining `indeterminate`) → anything else counts as In Progress.

Refresh via `npm run jira:workload` when `JIRA_EMAIL` / `JIRA_TOKEN` are set, or reproduce the same tallies with Atlassian MCP `searchJiraIssuesUsingJql`. `closedCount` is for reporting; it does not add to ETC.
...
1. Set workload fields on `jiraIssues[]` per §3 Workload (`todoCount`, `inProgressCount`, `codeReviewCount`, `qaReviewCount`, optional `closedCount`). If any workload field is present, the renderer computes a date from the same ETC unit formula as §3 ETC; otherwise it falls back to a manual `jiraIssues[].targetDate` string when provided.
...
3. Let W = ceil(ETC workload units), where units = To Do × 1.5 + In Progress × 1 + Code Review × 0.5 + QA Review × 0.25. Advance W working days forward (Mon–Fri only; weekends do not count).
...
2. Then set or refresh workload counts (`todoCount`, `inProgressCount`, `codeReviewCount`, `qaReviewCount`, `closedCount`; and optional `etc`) from Jira when you use workload-based ETC and derived % Cmp.
...
- ETC = (To Do × 1.5) + (In Progress × 1) + (Code Review × 0.5) + (QA Review × 0.25) whenever any workload count field is set on the issue (`todoCount`, `inProgressCount`, `codeReviewCount`, `qaReviewCount`, or `closedCount`); otherwise the renderer uses stored `etc` when present. Closed items do not increase ETC.
...
| Project Manager (@mentions)    | `owners[]`          | Use initials only (e.g. GY, SS).                                                                 |
...
Do not use paired asterisks for pseudo-bold or Markdown-style emphasis in merged JSON string fields (exec bullets, `jiraQuerySummary`, etc.). The renderer outputs plain text only; asterisks show up literally. Prefer straightforward wording for emphasis.
...
npm run render
# omit path: uses data/example-merged.json (override: argv, or EXEC_STATUS_MERGED in .env.local)
```
...
| Step                     | MCP / tool                                                                                                                                                                                                                                                                                   |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Pull Jira slice / fields | Atlassian → `searchJiraIssuesUsingJql`, `getJiraIssue` (include `customfield_*` from `JIRA_EAF_FIELD_ID` in `.env.example`) → `npm run jira:eaf -- --patch` — apply EAF per issue, then `npm run jira:workload` (or MCP + §3 Workload) for `todoCount` / `inProgressCount` / `codeReviewCount` / `qaReviewCount` / `closedCount` before ETC / % |
...
- HTML does not match the file you edited: confirm render used your path — terminal shows `Read …` with pick reason; View Source shows `merged input` and `pick:` in the first HTML comment. Default input is `data/example-merged.json`. Override with `npm run render -- data/your-merged.json` or `EXEC_STATUS_MERGED` in `.env.local`. Hard-refresh the browser when previewing `dist/status.html`.
